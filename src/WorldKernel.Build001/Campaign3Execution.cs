...
public sealed record Campaign3StateObservation(
    string Schema,
...
public sealed record Campaign3CheckOutcome(
    bool Observed,
    bool Started,
...
public sealed record Campaign3BrowserOutcome(
    bool Observed,
    string? PresentedHead,
    string? Href,
    JsonElement Evidence);

public sealed record Campaign3ProviderOutcome(
    string Schema,
    DateTimeOffset ObservedAt,
    Campaign3CheckOutcome Check,
    Campaign3BrowserOutcome Browser);

public sealed record Campaign3ResolvedOutcome(
...
public static class Campaign3OutcomeResolver
{
    public const string StateSchema = "world-kernel-build001-campaign3-state-observation-v1";
    public const string ProviderSchema = "world-kernel-build001-campaign3-provider-outcome-v1";

    public static Campaign3ResolvedOutcome Resolve(
        string semanticAction,
        Campaign3StateObservation before,
        Campaign3StateObservation after,
        bool receiptAccepted,
        Campaign3ProviderOutcome provider)
    {
        _ = Build001Contract.ForAction(semanticAction);
        if (before.Schema != StateSchema || after.Schema != StateSchema)
        {
            throw new InvalidDataException("Campaign 3 state observation schema mismatch.");
        }
        if (provider.Schema != ProviderSchema)
        {
            throw new InvalidDataException("Campaign 3 provider outcome schema mismatch.");
        }
        ...
        return new Campaign3ResolvedOutcome(actual, deltas, Array.Empty<string>());
    }
...
public sealed record Campaign3BeginInput(
    string Schema,
    string CampaignId,
    string Phase,
    string TrialId,
...
public sealed record Campaign3AcquisitionBlockRegistrationInput(
    string Schema,
    string CampaignId,
    string Phase,
    string ConfigurationBlockId,
...
public sealed record Campaign3AcquisitionBlockRegistrationRecord(
    string Schema,
    string CampaignId,
    string ConfigurationBlockId,
    string SeedId,
...
public sealed record Campaign3ResetRegistrationRecord(
    string Schema,
    string CampaignId,
    string ConfigurationBlockId,
    string SeedId,
    ...
    bool Passed,
    DateTimeOffset RegisteredAt);
public sealed record Campaign3BeginRecord(
    string Schema,
    string CampaignId,
    string Phase,
    string TrialId,
...
public sealed record Campaign3CloseRecord(
    string Schema,
    string CampaignId,
    string TrialId,
    string ConfigurationBlockId,
...
public sealed record Campaign3ActionCoverage(
    string SemanticAction,
    int EligibleClosedEpisodes,
    int DistinctConfigurationBlocks,
    int DistinctSeeds);

public sealed record Campaign3AcquisitionCoverage(
    string Schema,
    string CampaignId,
    DateTimeOffset GeneratedAt,
    int ConfigurationBlocks,
    IReadOnlyList<Campaign3ActionCoverage> Actions,
    int PushAccepted,
    int PushRejected,
...
{
    public const string CampaignId = "build001-campaign-3";
public const string BlockRegistrationInputSchema = "world-kernel-build001-campaign3-block-registration-input-v1";
    public const string BlockRegistrationRecordSchema = "world-kernel-build001-campaign3-block-registration-v1";
    public const string ResetRegistrationRecordSchema = "world-kernel-build001-campaign3-reset-registration-v1";
    public const string BeginInputSchema = "world-kernel-build001-campaign3-begin-input-v1";
    public const string BeginRecordSchema = "world-kernel-build001-campaign3-begin-record-v1";
    public const string CloseRecordSchema = "world-kernel-build001-campaign3-close-record-v1";
    public const string CoverageSchema = "world-kernel-build001-campaign3-acquisition-coverage-v1";
    public const string FreezeManifestSchema = "world-kernel-build001-campaign3-execution-freeze-v2";
    public const string SubjectResultSchema = "world-kernel-build001-campaign3-subject-adapter-result-v1";
    ...
    public static async Task<Campaign3AcquisitionBlockRegistrationRecord> RegisterAcquisitionBlockAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        PreflightGateEvaluator.EnsurePhaseAuthorized(
            Path.Combine(root, "artifacts", "campaign-3", "preflight", "preflight-gates.json"),
            "acquisition");
        ...
        var campaignRoot = Path.Combine(root, "experiments", "build001", "campaign-3");
        var outputFile = EnsureUnder(campaignRoot, outputPath);
        EnsureAbsent(outputFile);
        var input = Deserialize<Campaign3AcquisitionBlockRegistrationInput>(
        ...
        {
            throw new InvalidDataException("Campaign 3 acquisition block registration is invalid.");
        }
        ...
        if (existing.Count > 1) throw new InvalidDataException("Prospective Campaign 3 seed commitment is duplicated.");
        if (existing.Count == 1)
        {
            var row = existing[0];
        ...
        var record = new Campaign3AcquisitionBlockRegistrationRecord(
    ...
    public static async Task<Campaign3ResetRegistrationRecord> RegisterAcquisitionResetAsync(
        string repositoryRoot,
        string secretFile,
        string configurationBlockId,
        string seedId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (RequiredString(verification, "schema") != "world-kernel-build001-campaign3-independent-reset-verification-v1" ||
        ...
        var hiddenInput = Deserialize<Campaign3AcquisitionBlockRegistrationInput>(
        ...
        if (hiddenCount > 1 || resetCount > 1 || hiddenCount != resetCount)
            throw new InvalidDataException("Campaign 3 reset registration is partially duplicated or inconsistent.");
        DateTimeOffset registeredAt;
        if (hiddenCount == 1)
        {
            ...
            if (CanonicalJson.HashJson(existingHidden.RootElement) != hiddenHash || existingHiddenFingerprint != expectedFingerprint ||
                ...
                throw new InvalidDataException("Existing Campaign 3 reset registration differs from the verified retry input.");
            registeredAt = row.RecordedAt;
        }
        else
        {
            ...
            await using (var hidden = new NpgsqlCommand("""
                ...
                VALUES (@id,@seed,'acquisition-action-slot',@configuration,@fingerprint,'campaign3-acquisition-schedule-v2');
        ...
        var record = new Campaign3ResetRegistrationRecord(
    ...
    public static async Task<Campaign3BeginRecord> BeginAsync(
        string repositoryRoot,
        string secretFile,
        string evidenceRoot,
        string inputPath,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var outputFile = EnsureUnder(campaignRoot, outputPath);
        EnsureAbsent(outputFile);
        var input = Deserialize<Campaign3BeginInput>(await File.ReadAllBytesAsync(inputFile, cancellationToken).ConfigureAwait(false));
        ...
        var pre = Deserialize<Campaign3StateObservation>(preBytes);
        ...
        var preEvidence = await store.PutAsync(preBytes, "git/native", "campaign3-state-observer",
        ...
        var requestEvidence = await store.PutAsync(requestBytes, "chatgpt/product", "campaign3-request-builder",
        ...
        var subjectEvidence = await store.PutAsync(subjectBytes, "chatgpt/product", "campaign3-subject-adapter",
        ...
        var preObservation = new ObservationRecord(
            preObservationId,
            localId,
            "campaign3-state-observer",
            "campaign3-state-observation-v1",
            "git/native",
            pre.ObservedAt,
            "succeeded",
        ...
        var predictionDeclaration = new PredictionDeclaration(
            ...
            "fresh-campaign3-temporary-chat",
            Campaign3Attestation.FreshInvocationMethodVersion,
            producer);
        var defects = await database.CommitPredictionAsync(predictionDeclaration, cancellationToken).ConfigureAwait(false);
        if (defects.Count != 0)
        {
            throw new InvalidDataException("Validated Campaign 3 subject prediction acquired format defects: " + string.Join("; ", defects));
        }
        ...
        var record = new Campaign3BeginRecord(
            BeginRecordSchema,
            CampaignId,
            input.Phase,
            input.TrialId,
    ...
    public static async Task<Campaign3CloseRecord> CloseAsync(
        string repositoryRoot,
        string secretFile,
        string evidenceRoot,
        string beginPath,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var outputFile = EnsureUnder(campaignRoot, outputPath);
        EnsureAbsent(outputFile);
        var begin = Deserialize<Campaign3BeginRecord>(await File.ReadAllBytesAsync(beginFile, cancellationToken).ConfigureAwait(false));
        if (begin.Schema != BeginRecordSchema || begin.CampaignId != CampaignId || begin.Phase != "acquisition" || begin.Arm != "acquisition")
        {
            throw new InvalidDataException("Campaign 3 begin record is not an acquisition dispatch.");
        }
        var before = Deserialize<Campaign3StateObservation>(await File.ReadAllBytesAsync(begin.PreObservationPath, cancellationToken).ConfigureAwait(false));
        ...
        var post = Deserialize<Campaign3StateObservation>(postBytes);
        var provider = Deserialize<Campaign3ProviderOutcome>(providerBytes);
        ...
        var resolved = Campaign3OutcomeResolver.Resolve(begin.SemanticAction, before, post, receiptAccepted, provider);
        ...
        {
            throw new InvalidDataException("A complete Campaign 3 acquisition outcome unexpectedly became ineligible.");
        }
        ...
        var postEvidence = await store.PutAsync(postBytes, "git/native", "campaign3-state-observer",
        ...
        var providerEvidence = await store.PutAsync(providerBytes, "github/provider", "campaign3-provider-outcome-observer",
        ...
        var providerObservation = new ObservationRecord(
            ...
            "campaign3-provider-outcome-observer",
            "campaign3-provider-outcome-v1",
            "github/provider",
            provider.ObservedAt,
            "succeeded",
        ...
        await database.WithConnectionAsync(async (connection, token) =>
        {
            ...
            await using (var outcome = new NpgsqlCommand("""
                INSERT INTO wk.outcome(
                  ...
                  'consistent_with_action','campaign3-outcome-resolver-v1',clock_timestamp());
                """, connection, transaction))
            {
                ...
                """, connection, transaction))
            {
                ...
                KernelDb.AddJson(episode, "versions", JsonSerializer.SerializeToElement(new
                {
                    ...
                    subject_adapter = Campaign3Attestation.FreshInvocationMethodVersion
                }, JsonDefaults.Options));
        ...
        var record = new Campaign3CloseRecord(
            CloseRecordSchema,
            CampaignId,
            begin.TrialId,
    ...
    public static async Task<Campaign3AcquisitionCoverage> WriteCoverageAsync(
        string repositoryRoot,
        string secretFile,
        string outputPath,
        CancellationToken cancellationToken = default)
    {
        var root = Path.GetFullPath(repositoryRoot);
        var output = EnsureUnder(Path.Combine(root, "experiments", "build001", "campaign-3"), outputPath);
        await using var database = new KernelDb(ConnectionSecrets.ReadConnectionString(secretFile, "owner_connection"));
        ...
        var actionCoverage = Build001Contract.Propositions.Keys.OrderBy(value => value, StringComparer.Ordinal).Select(action =>
        {
            ...
            return new Campaign3ActionCoverage(
                action,
                actionRows.Length,
        ...
        var coverage = new Campaign3AcquisitionCoverage(
            CoverageSchema,
            CampaignId,
            DateTimeOffset.UtcNow,
            blocks,
    ...
    private static async Task<Campaign3CloseRecord?> TryRecoverExistingCloseAsync(
        string root,
        string secretFile,
        Campaign3BeginRecord begin,
        Campaign3StateObservation before,
        byte[] receiptBytes,
        byte[] postBytes,
        byte[] providerBytes,
        Campaign3StateObservation post,
        Campaign3ProviderOutcome provider,
        Campaign3ResolvedOutcome resolved,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var existing = await database.WithConnectionAsync(async (connection, token) =>
        {
                ...
                """, connection))
            {
                ...
                if (await reader.ReadAsync(token).ConfigureAwait(false))
                {
                    ...
                    if (await reader.ReadAsync(token).ConfigureAwait(false))
                        throw new InvalidDataException("Campaign 3 Action has duplicate closed TransitionEpisodes.");
                }
            }
        ...
        using (var brierDocument = JsonDocument.Parse(existing.BrierComponentsJson))
        {
            if (existing.EligibilityStatus != score.EligibilityStatus ||
                ...
                throw new InvalidDataException("Existing closed Campaign 3 episode differs from fresh independently resolved evidence.");
        }
        if (existing.PostClaimIds.Count == 0)
            throw new InvalidDataException("Existing closed Campaign 3 episode is missing typed post-action Claims.");
        ...
        if (File.Exists(publicEpisodePath))
        {
            ...
            if (CanonicalJson.Sha256(actualBytes) != expectedHash)
                throw new InvalidDataException("Existing public Campaign 3 episode differs from recovered durable state.");
        }
        else
        {
            await WriteNewAsync(publicEpisodePath, expectedBytes, cancellationToken).ConfigureAwait(false);
        }
        return new Campaign3CloseRecord(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (rows.Count > 1) throw new InvalidDataException("Evaluator ground truth is duplicated for one Campaign 3 Action.");
        if (rows.Count == 1)
        {
            ...
            if (rows[0].Resolver != "campaign3-outcome-resolver-v1" ||
        ...
        await using var command = new NpgsqlCommand("""
            ...
            VALUES (@id,@action,@block,'locked',@actual,@deltas,@invariants,@hashes,'campaign3-outcome-resolver-v1',@resolved);
            """, connection);
    ...
    private static async Task<Campaign3BeginRecord?> TryRecoverExistingBeginAsync(
        string root,
        string secretFile,
        Campaign3BeginInput input,
        byte[] resetBytes,
        JsonElement reset,
        byte[] preBytes,
        Campaign3StateObservation pre,
        byte[] requestBytes,
        byte[] subjectBytes,
        IReadOnlyDictionary<string, double?> prediction,
        CancellationToken cancellationToken)
    {
        ...
        return await database.WithConnectionAsync(async (connection, token) =>
        {
            ...
            if (actions.Count != 1) throw new InvalidDataException("Campaign 3 begin recovery found duplicate durable Actions for one trial.");
            var action = actions[0];
            if (action.SemanticAction != input.SemanticAction || action.ParametersHash != CanonicalJson.HashJson(input.Parameters))
                throw new InvalidDataException("Existing Campaign 3 Action differs from the sealed retry input.");
            ...
            if (targets.Length != 2) throw new InvalidDataException("Existing Campaign 3 Action has an unexpected target set.");
            ...
            async Task<Guid> FindEvidenceAsync(string hash, string observer, string method)
            {
                ...
                return await command.ExecuteScalarAsync(token).ConfigureAwait(false) is Guid id
                    ? id : throw new InvalidDataException($"Campaign 3 recovery is missing durable {observer}/{method} evidence.");
            }
            var preEvidenceId = await FindEvidenceAsync(CanonicalJson.Sha256(preBytes), "campaign3-state-observer", "fresh-pre-dispatch-observation").ConfigureAwait(false);
            var preRemoteEvidenceId = await FindEvidenceAsync(CanonicalJson.Sha256(CanonicalJson.Serialize(BuildRemoteRefProviderPayload(pre))), "campaign3-github-ref-observer", "git-ls-remote-exact-hosted-ref").ConfigureAwait(false);
            var requestEvidenceId = await FindEvidenceAsync(CanonicalJson.Sha256(requestBytes), "campaign3-request-builder", "locked-subject-request").ConfigureAwait(false);
            var subjectEvidenceId = await FindEvidenceAsync(CanonicalJson.Sha256(subjectBytes), "campaign3-subject-adapter", "fresh-temporary-chat").ConfigureAwait(false);

            Guid preObservationId;
            await using (var command = new NpgsqlCommand("""
                SELECT observation_id FROM wk.observation
                WHERE target_manifestation_id=@target AND observer_name='campaign3-state-observer' AND observed_at=@observed
                ORDER BY recorded_at LIMIT 1;
                """, connection))
            {
                ...
                preObservationId = await command.ExecuteScalarAsync(token).ConfigureAwait(false) is Guid id
                    ? id : throw new InvalidDataException("Campaign 3 recovery is missing the pre-dispatch Observation.");
            }

            Guid preRemoteObservationId;
            await using (var command = new NpgsqlCommand("""
                SELECT observation_id FROM wk.observation
                WHERE target_manifestation_id=@target AND observer_name='campaign3-github-ref-observer' AND observed_at=@observed
                ORDER BY recorded_at LIMIT 1;
                """, connection))
            {
                ...
                preRemoteObservationId = await command.ExecuteScalarAsync(token).ConfigureAwait(false) is Guid id
                    ? id : throw new InvalidDataException("Campaign 3 recovery is missing the hosted-ref pre-dispatch Observation.");
            }
            ...
            if (preClaims.Count == 0) throw new InvalidDataException("Campaign 3 recovery is missing typed pre-action Claims.");
                ...
                """, connection))
            {
                ...
                correspondenceId = await command.ExecuteScalarAsync(token).ConfigureAwait(false) is Guid id
                    ? id : throw new InvalidDataException("Campaign 3 recovery is missing the conservative correspondence record.");
            }
            ...
            await using (var command = new NpgsqlCommand("SELECT prediction_id,outcome_probabilities::text FROM wk.prediction WHERE action_id=@action;", connection))
            {
                ...
                if (await reader.ReadAsync(token).ConfigureAwait(false))
                {
                    ...
                    if (await reader.ReadAsync(token).ConfigureAwait(false)) throw new InvalidDataException("Campaign 3 Action has duplicate Predictions.");
                }
            ...
            if (predictionId == Guid.Empty)
            {
                ...
                var declaration = new PredictionDeclaration(
                    ...
                    "fresh-campaign3-temporary-chat", Campaign3Attestation.FreshInvocationMethodVersion, producer);
            ...
            if (dispatchCount > 1) throw new InvalidDataException("Campaign 3 Action has duplicate dispatch seals.");
            if (dispatchCount == 0)
            {
            ...
            return new Campaign3BeginRecord(
    ...
    private static IReadOnlyList<string> BuildPublicObservedFacts(Campaign3StateObservation state) =>
    [
    ...
    private static IReadOnlyList<PublicClaimExport> BuildPublicClaimExports(
        IReadOnlyDictionary<string, Guid> ids, Campaign3StateObservation state, Campaign3ProviderOutcome? provider,
        string localRef, string stateEvidenceHash, string remoteRefEvidenceHash, string? providerEvidenceHash, string disposition)
    {
        ...
        foreach (var pair in ids.OrderBy(value => value.Key, StringComparer.Ordinal))
        {
            ...
            var value = pair.Key switch
            {
                ...
                _ => throw new InvalidDataException("Unknown Campaign 3 public claim key: " + pair.Key)
            };
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (existing.Count > 1) throw new InvalidDataException("Campaign 3 evidence is duplicated for identical content/provenance.");
    ...
    private static async Task<Guid> EnsureLocalManifestationAsync(
        ...
        CancellationToken cancellationToken)
    {
        var incarnation = $"campaign3:{input.TrialId}:{generationId}";
        var existing = await database.WithConnectionAsync(async (connection, token) =>
        {
            var ids = new List<Guid>();
            await using var command = new NpgsqlCommand("""
        ...
        if (existing.Count > 1) throw new InvalidDataException("Campaign 3 local manifestation incarnation is duplicated.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (rows.Count > 1) throw new InvalidDataException("Campaign 3 Observation is duplicated for one target/time/observer.");
        if (rows.Count == 1)
        {
            ...
            if (candidate.RawNormalizedPayload is not JsonElement rawPayload ||
                ...
                throw new InvalidDataException("Existing Campaign 3 Observation differs from the retry artifact.");
    ...
    private static async Task<IReadOnlyDictionary<string, Guid>> InsertStateClaimsAsync(
        ...
        Campaign3StateObservation state, Guid remoteRefObservationId, Guid remoteRefEvidenceId,
        Campaign3ProviderOutcome? provider, Guid? providerObservationId, Guid? providerEvidenceId, CancellationToken cancellationToken)
    {
        ...
        if (existing.Count != 0)
        {
            if (!existing.Keys.ToHashSet(StringComparer.Ordinal).SetEquals(expectedKeys))
                throw new InvalidDataException("Existing Campaign 3 typed Claims are incomplete or differ from the retry observation.");
            return existing;
        }
        await database.WithConnectionAsync(async (connection, token) =>
        {
        ...
        string branch, Guid observationId, Guid evidenceId, CancellationToken cancellationToken)
    {
            ...
            """, connection, transaction))
        {
            ...
            KernelDb.AddJson(command, "producer", JsonSerializer.SerializeToElement(new { component = "campaign3-execution", version = "v1" }, JsonDefaults.Options));
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (existing.Count > 1) throw new InvalidDataException("Campaign 3 correspondence is duplicated for one reset incarnation.");
        if (existing.Count == 1) return existing[0];
        var id = Guid.NewGuid();
        await database.WithConnectionAsync(async (connection, token) =>
        {
            ...
            await using (var command = new NpgsqlCommand("""
                ...
                ) VALUES (@id,@left,'git','working_copy_of',@right,'campaign3-provider-reset',1.0,'candidate',
    ...
    {
        var path = Path.Combine(root, "experiments", "build001", "campaign-3", "preregistration-freeze-manifest.json");
        var sidecar = path + ".sha256";
        var actual = CanonicalJson.Sha256(File.ReadAllBytes(path));
        if (!File.Exists(sidecar)) throw new InvalidDataException("Campaign 3 freeze manifest SHA-256 sidecar is absent.");
        var expected = File.ReadAllText(sidecar).Trim().Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries)[0];
        if (!string.Equals(actual, expected, StringComparison.Ordinal))
            throw new InvalidDataException($"Campaign 3 freeze manifest hash mismatch: {actual}.");
        return actual;
    }
    ...
    private static void ValidateFreeze(string root)
    {
        ...
        var singleProspectiveFreeze = value.TryGetProperty("single_prospective_freeze", out var singleFreezeValue) &&
                                      singleFreezeValue.ValueKind == JsonValueKind.True;
        var frozenBeforeAcquisition = value.TryGetProperty("frozen_before_acquisition", out var beforeAcquisitionValue) &&
                                      beforeAcquisitionValue.ValueKind == JsonValueKind.True;
        var frozenBeforeFirstSubject = value.TryGetProperty("frozen_before_first_subject_invocation", out var beforeSubjectValue) &&
                                       beforeSubjectValue.ValueKind == JsonValueKind.True;
        var zeroScienceAtFreeze = value.TryGetProperty("scientific_subjects_run", out var scienceValue) &&
                                  scienceValue.ValueKind == JsonValueKind.False;
        var forbiddenReplacementMarker = value.TryGetProperty("repair_after_seed_registration", out _) ||
                                         value.TryGetProperty("supersedes_execution_freeze_commit", out _) ||
                                         value.TryGetProperty("replacement_freeze", out _);
        if (RequiredString(value, "schema") != FreezeManifestSchema || RequiredString(value, "campaign_id") != CampaignId ||
            !RequiredBoolean(value, "valid") || !singleProspectiveFreeze || !frozenBeforeAcquisition ||
            !frozenBeforeFirstSubject || !zeroScienceAtFreeze)
            throw new InvalidDataException("Campaign 3 execution freeze manifest is not the single valid prospective pre-science freeze.");
        if (forbiddenReplacementMarker)
            throw new InvalidDataException("Campaign 3 forbids replacement/superseding execution-freeze markers.");
        ...
        if (commit.Length != 40 || commit.Any(character => !Uri.IsHexDigit(character)))
            throw new InvalidDataException("Campaign 3 execution implementation commit is invalid.");
        var files = implementation.GetProperty("frozen_files");
        if (files.ValueKind != JsonValueKind.Object || !files.EnumerateObject().Any())
            throw new InvalidDataException("Campaign 3 execution freeze contains no frozen implementation files.");
    ...
    private static void ValidateBeginInput(Campaign3BeginInput input)
    {
        if (input.Schema != BeginInputSchema || input.CampaignId != CampaignId || input.Phase != "acquisition" || input.Arm != "acquisition")
        {
            throw new InvalidDataException("Campaign 3 begin input is outside the authorized acquisition phase.");
        }
        ...
        {
            throw new InvalidDataException("Campaign 3 begin input is incomplete.");
        }
    }
    ...
    {
        if (RequiredString(hidden, "schedule_version") != "campaign3-acquisition-action-slot-v2")
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException("Campaign 3 dispatch lacks a unique prospective evaluator seed/reset registration.");
        }
    }
    private static void ValidateReset(Campaign3BeginInput input, JsonElement reset, Campaign3StateObservation pre)
    {
        if (RequiredString(reset, "reset_version") != "build001-fixture-reset-v1" ||
            RequiredString(reset, "phase") != "acquisition" || RequiredString(reset, "arm") != "acquisition" ||
            !RequiredBoolean(reset, "reset_verified"))
        {
            throw new InvalidDataException("Fixture reset was not verified for Campaign 3 acquisition.");
        }
        var material = reset.GetProperty("material");
        if (RequiredString(material, "repository") != FixtureRepository ||
        ...
        JsonElement result)
    {
        ...
        {
            throw new InvalidDataException("Campaign 3 subject adapter did not produce a valid fresh invocation.");
        }
    ...
    private static string EnsureUnder(string parent, string child)
    {
        ...
        {
            throw new InvalidOperationException($"Campaign 3 path escapes the experiment namespace: {path}");
        }
        return path;
    }
    ...
    {
        if (File.Exists(path)) throw new IOException($"Campaign 3 immutable output already exists: {path}");
    }
