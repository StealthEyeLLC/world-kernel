...
public sealed record Campaign2LineageHarnessResult(
    string Schema,
...
{
    public static async Task<Campaign2LineageHarnessResult> RunPrefreezeLineageHarnessAsync(
        string secretFile,
        string evidenceRoot,
        string workingCopy,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var state = Deserialize<Campaign2StateObservation>(stateBytes);
        _ = Build001Contract.ForAction(semanticAction);
        var trialId = $"c2r-harness-{semanticAction.Replace(':', '-')}-{Guid.NewGuid():N}";
        var blockId = $"c2r-harness-{Guid.NewGuid():N}";
        ...
        var input = new Campaign2BeginInput(
        ...
        var localEvidence = await store.PutAsync(
            stateBytes, "git/native", "campaign2-state-observer", "application/json",
        ...
        var localObservation = await EnsureObservationAsync(database, new ObservationRecord(
            Guid.NewGuid(), localId, "campaign2-state-observer", "campaign2r-prefreeze-lineage-harness-v1", "git/native",
        ...
        var correspondenceId = await InsertCorrespondenceAsync(
            ...
            CanonicalJson.Sha256Utf8("campaign2r-prefreeze-lineage-harness|" + trialId), cancellationToken).ConfigureAwait(false);

        var producer = JsonSerializer.SerializeToElement(new
        {
            mechanism = "campaign2r-prefreeze-lineage-harness",
            non_scientific_harness = true,
            scored = false,
        ...
        var defects = await database.CommitPredictionAsync(new PredictionDeclaration(
            ...
            Build001Contract.DefaultHorizons(), "campaign2r-prefreeze-lineage-harness", "campaign2r-prefreeze-lineage-harness-v1", producer),
        ...
        return await database.WithConnectionAsync(async (connection, token) =>
        {
            ...
            return new Campaign2LineageHarnessResult(
                "world-kernel-build001-campaign2r-prefreeze-lineage-harness-v1", semanticAction, trialId,
    ...
    private static JsonElement HarnessParameters(string semanticAction, Campaign2StateObservation state) => semanticAction switch
    {
        "git:create_local_commit" => JsonSerializer.SerializeToElement(new { relative_path = "fixture/state.txt", message = "Campaign 2R prefreeze harness local commit", timestamp = DateTimeOffset.UtcNow }),
        "git:create_branch" => JsonSerializer.SerializeToElement(new { branch = "wk-b001-c2r-harness-new" }),
        "git:push_ref" => JsonSerializer.SerializeToElement(new { branch = state.Branch }),
        "github:create_remote_commit" => JsonSerializer.SerializeToElement(new { branch = state.Branch, file = "fixture/state.txt", text = "campaign2r-prefreeze-harness", message = "Campaign 2R prefreeze harness remote commit" }),
