...
public static partial class Campaign2Execution
{
    ...
    private static JsonElement BuildRemoteRefProviderPayload(Campaign2StateObservation state)
    {
        if (state.Commands.ValueKind != JsonValueKind.Array)
            throw new InvalidDataException("Campaign 2R state observation does not retain the provider command transcript.");
        ...
        foreach (var command in state.Commands.EnumerateArray())
        {
            ...
            if (values.SequenceEqual(expectedArguments, StringComparer.Ordinal))
            {
                if (matched is not null)
                    throw new InvalidDataException("Campaign 2R state observation contains duplicate hosted-ref provider queries.");
                matched = command.Clone();
            }
        }

        if (matched is not JsonElement providerCommand)
            throw new InvalidDataException("Campaign 2R state observation lacks the exact hosted-ref provider query.");
        if (!providerCommand.TryGetProperty("exit_code", out var exitCodeElement) ||
            ...
            throw new InvalidDataException("Campaign 2R hosted-ref provider query did not succeed.");
        if (!providerCommand.TryGetProperty("output", out var outputElement) || outputElement.ValueKind != JsonValueKind.Array)
            throw new InvalidDataException("Campaign 2R hosted-ref provider query lacks retained output.");
        ...
        var output = outputElement.EnumerateArray()
            ...
            .ToArray();
        if (output.Length > 1)
            throw new InvalidDataException("Campaign 2R hosted-ref provider query returned more than one exact ref.");
        ...
        if (output.Length == 1)
        {
            ...
            if (parts.Length != 2 || parts[0].Length != 40 || parts[0].Any(character => !Uri.IsHexDigit(character)) ||
                ...
                throw new InvalidDataException("Campaign 2R hosted-ref provider query returned an invalid exact-ref row.");
            providerHead = parts[0].ToLowerInvariant();
        }

        if (!string.Equals(providerHead, state.RemoteHead, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Campaign 2R retained remote_head differs from the hosted-ref provider query.");

        return JsonSerializer.SerializeToElement(new
        {
            schema = "world-kernel-build001-campaign2r-hosted-ref-observation-v1",
    ...
    private static async Task<RemoteRefLineage> EnsureRemoteRefLineageAsync(
        ...
        Campaign2StateObservation state,
        CancellationToken cancellationToken)
    {
        ...
        var observation = new ObservationRecord(
            ...
            "campaign2-github-ref-observer",
            "campaign2r-github-ref-observation-v1",
            "github/provider",
            state.ObservedAt,
            "succeeded",
