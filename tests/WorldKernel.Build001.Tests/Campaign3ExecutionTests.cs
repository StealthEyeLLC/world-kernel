...
internal static class Campaign3ExecutionTests
{
    ...
    {
        var result = Campaign3OutcomeResolver.Resolve(action, before, after, accepted, provider);
    ...
    private static Campaign3StateObservation State(
        ...
        int parents = 1) => new(
        Campaign3OutcomeResolver.StateSchema,
    ...
    private static Campaign3ProviderOutcome Provider(
        bool checkStarted = false,
        bool checkSuccess = false,
        string? presentedHead = null) => new(
        Campaign3OutcomeResolver.ProviderSchema,
        DateTimeOffset.UtcNow,
        new Campaign3CheckOutcome(true, checkStarted, checkSuccess, checkSuccess ? "success" : null, JsonDefaults.EmptyArray),
        new Campaign3BrowserOutcome(presentedHead is not null, presentedHead, null, JsonDefaults.EmptyObject));
