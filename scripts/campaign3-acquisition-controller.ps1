param(
    ...
    [string] $SecretFile = 'C:\WorldKernel\Build001\campaign3-runtime\secrets\connections.json',
    [string] $EvidenceRoot = 'X:\WorldKernel\Build001\campaign3-evidence\blobs',
...
if (-not $env:WORLD_KERNEL_BUILD001_RUNTIME_ROOT) { $env:WORLD_KERNEL_BUILD001_RUNTIME_ROOT = 'C:\WorldKernel\Build001\campaign3-runtime' }
Set-StrictMode -Version Latest
$repoRoot = [IO.Path]::GetFullPath($RepositoryRoot)
$campaignRoot = Join-Path $repoRoot 'experiments\build001\campaign-3'
$acquisitionRoot = Join-Path $campaignRoot 'acquisition'
$evaluatorHiddenRoot = 'C:\WorldKernel\Build001\evaluator\campaign3\acquisition'
$fixturePublicRevision = '519d05879314cab45280a9f58efbd8859ecd8d64'
$worker = Join-Path $repoRoot 'scripts\campaign3-user-worker.ps1'
$observe = Join-Path $repoRoot 'scripts\campaign3-observe-state.ps1'
$requestBuilder = Join-Path $repoRoot 'scripts\campaign3-new-subject-request.ps1'
$resetScript = Join-Path $repoRoot 'scripts\fixture-reset.ps1'
$verifyResetScript = Join-Path $repoRoot 'scripts\campaign3-verify-reset.ps1'
$prepareActionScript = Join-Path $repoRoot 'scripts\campaign3-prepare-action.ps1'
$recoverActionScript = Join-Path $repoRoot 'scripts\campaign3-recover-action.ps1'
...
$preflight = Join-Path $repoRoot 'artifacts\campaign-3\preflight\preflight-gates.json'
...
if ($InitialBlocks -ne 24 -or $MaximumBlocks -ne 36) { throw 'Campaign 3 acquisition block bounds are frozen at 24 and 36.' }
foreach ($required in @($worker,$observe,$requestBuilder,$resetScript,$verifyResetScript,$prepareActionScript,$recoverActionScript,$refObserver,$checkObserver,$cliDll,$node,$sdk,$preflight,$coldPackage,$SecretFile)) {
    if (-not (Test-Path $required)) { throw "Required Campaign 3 acquisition dependency is absent: $required" }
}
...
function Write-NewJson([string] $Path, [object] $Value) {
    if (Test-Path $Path) { throw "Immutable Campaign 3 artifact already exists: $Path" }
...
function Invoke-Cli([string[]] $Arguments) {
    ...
    if ($exit -ne 0) { throw "Campaign 3 CLI failed: $($lines -join [Environment]::NewLine)" }
    $json = $lines | Where-Object { $_.Trim().StartsWith('{') } | Select-Object -Last 1
    if (-not $json) { throw 'Campaign 3 CLI returned no JSON result.' }
...
function Invoke-UserJob([string] $Operation, [object] $Arguments, [ValidateSet('Limited','Highest')] [string] $RunLevel, [string] $JobDirectory, [int] $TimeoutSeconds) {
    ...
    $job = [ordered]@{
        schema = 'world-kernel-build001-campaign3-user-job-v1'
        job_id = $jobId
        operation = $Operation
    ...
    try {
        ...
        if ($state -in @('Running','Queued')) { throw "Campaign 3 user job timed out: $Operation" }
        if (-not (Test-Path $statusPath)) { throw "Campaign 3 user job wrote no status: $Operation" }
        $status = Get-Content -Raw -LiteralPath $statusPath | ConvertFrom-Json
        if (-not $status.ok) { throw "Campaign 3 user job failed ($Operation): $($status.error)" }
        return $status
    }
    finally {
...
for ($block = 1; $block -le $MaximumBlocks; $block++) {
    $configurationBlockId = 'c3-acq-{0:d2}' -f $block
    $blockSeed = 'campaign3-acquisition-block-seed-{0:d2}' -f $block
    ...
    foreach ($semanticAction in $actions) {
        ...
        $resetBlockId = "c3-a{0:d2}-$code" -f $block
        $resetBranch = "wk-b001-c3-a{0:d2}-$code" -f $block
        ...
        if (-not (Test-Path $registrationPath)) {
            ...
            $hiddenConfiguration = [ordered]@{
                schedule_version = 'campaign3-acquisition-action-slot-v2'
            ...
            Write-NewJson $hiddenInputPath ([ordered]@{
                schema = 'world-kernel-build001-campaign3-block-registration-input-v1'
            ...
            Invoke-Cli @('campaign3-register-block','--repo-root',$repoRoot,'--secret-file',$SecretFile,
                '--input',$hiddenInputPath,'--output',$registrationPath) | Out-Null
        }

        Write-ReplaceJson $controllerStatus ([ordered]@{
            schema = 'world-kernel-build001-campaign3-controller-status-v1'
            state = 'running'
            block = $block
            trial_id = $trialId
            semantic_action = $semanticAction
        ...
        if (-not (Test-Path $resetRegistrationPath)) {
            Invoke-Cli @('campaign3-register-reset','--repo-root',$repoRoot,'--secret-file',$SecretFile,
        ...
        if (-not (Test-Path $beginInputPath)) {
            Write-NewJson $beginInputPath ([ordered]@{
                schema = 'world-kernel-build001-campaign3-begin-input-v1'
                campaign_id = 'build001-campaign-3'
                phase = 'acquisition'
                trial_id = $trialId
                configuration_block_id = $configurationBlockId
        ...
        Invoke-Cli @('campaign3-begin','--repo-root',$repoRoot,'--secret-file',$SecretFile,'--evidence-root',$EvidenceRoot,
        ...
        if (-not (Test-Path $preparePath) -or -not (Test-Path $prePath)) {
            throw "Sealed Campaign 3 trial is missing its immutable preparation/pre-observation artifacts: $trialId"
        }
        $prepare = Get-Content -Raw -LiteralPath $preparePath | ConvertFrom-Json
        $actionBranch = [string]$prepare.action_branch
        ...
        if ($semanticAction -eq 'git:push_ref') {
            ...
            if (-not (Test-Path $checkPath)) {
                ...
                $checkArtifact = [ordered]@{
                    schema = 'world-kernel-build001-campaign3-check-observation-v1'
        ...
        if (-not (Test-Path $providerPath)) {
            Write-NewJson $providerPath ([ordered]@{
                schema = 'world-kernel-build001-campaign3-provider-outcome-v1'
                observed_at = [DateTimeOffset]::UtcNow.ToString('O')
                check = $check
                browser = $browser
            })
        }
        Invoke-Cli @('campaign3-close','--repo-root',$repoRoot,'--secret-file',$SecretFile,'--evidence-root',$EvidenceRoot,
            '--begin',$beginPath,'--receipt',$receiptPath,'--post-observation',$postPath,'--provider-outcome',$providerPath,'--output',$closePath) | Out-Null
    }

    Invoke-Cli @('campaign3-coverage','--repo-root',$repoRoot,'--secret-file',$SecretFile,'--output',$coveragePath) | Out-Null
...
if (-not $finalCoverage.stop_rule_satisfied) {
    Write-ReplaceJson $controllerStatus ([ordered]@{
        schema = 'world-kernel-build001-campaign3-controller-status-v1'; state = 'blocked_at_maximum'
        coverage = $finalCoverage; updated_at = [DateTimeOffset]::UtcNow.ToString('O')
    })
    throw 'Campaign 3 acquisition coverage was not achieved by the frozen 36-block maximum.'
}
...
Write-NewJson $completePath ([ordered]@{
    schema = 'world-kernel-build001-campaign3-acquisition-complete-v1'
    campaign_id = 'build001-campaign-3'
...
Write-ReplaceJson $controllerStatus ([ordered]@{
    schema = 'world-kernel-build001-campaign3-controller-status-v1'; state = 'completed'
