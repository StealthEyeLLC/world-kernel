...
if (-not $output.StartsWith($campaignRoot, [StringComparison]::OrdinalIgnoreCase)) { throw 'Action preparation output escapes Campaign 2.' }
...
function New-RemoteAhead([string] $Role) {
    ...
    New-Commit $controllerRoot "Campaign 2 evaluator remote setup $ResetBranch" ([DateTimeOffset]::Parse($timestamp).AddSeconds(2).ToString('O'))
...
switch ($SemanticAction) {
    'git:create_local_commit' {
        ...
        $scheduled = [ordered]@{ relative_path = 'fixture/state.txt'; message = "Campaign 2 measured local commit $ResetBranch"; timestamp = $timestamp }
    ...
    'git:push_ref' {
        ...
        New-Commit $workspace "Campaign 2 evaluator push setup $ResetBranch" $timestamp
    ...
    'git:integrate_fast_forward' {
        if ($IntegrationRegime -eq 'rejected_non_fast_forward') {
            ...
            New-Commit $workspace "Campaign 2 evaluator local divergence $ResetBranch" $timestamp
        }
...
$commands += $remoteHead
$result = [ordered]@{
    schema = 'world-kernel-build001-campaign2-action-preparation-v1'
