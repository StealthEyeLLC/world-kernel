...
if (-not $jobFile.StartsWith($campaignRoot, [StringComparison]::OrdinalIgnoreCase)) { throw 'Campaign 3 worker job path escapes the experiment namespace.' }
$job = Get-Content -Raw -LiteralPath $jobFile | ConvertFrom-Json
if ($job.schema -ne 'world-kernel-build001-campaign3-user-job-v1') { throw 'Campaign 3 worker job schema mismatch.' }
$statusPath = [IO.Path]::GetFullPath([string]$job.status_path)
if (-not $statusPath.StartsWith($campaignRoot, [StringComparison]::OrdinalIgnoreCase)) { throw 'Campaign 3 worker status path escapes the experiment namespace.' }
if (Test-Path $statusPath) { throw "Campaign 3 worker status already exists: $statusPath" }
...
function Write-NewJson([string] $Path, [object] $Value) {
    ...
    if (-not $target.StartsWith($campaignRoot, [StringComparison]::OrdinalIgnoreCase)) { throw 'Campaign 3 worker output escapes the experiment namespace.' }
    if (Test-Path $target) { throw "Campaign 3 worker output already exists: $target" }
...
try {
    ...
    switch ([string]$job.operation) {
        ...
        'verify-reset' {
            & (Join-Path $PSScriptRoot 'campaign3-verify-reset.ps1') -WorkingCopy $a.working_copy -Branch $a.branch `
                -BrowserFreshness $a.browser_freshness -OutputPath $a.output_path | Out-Null
        }
        'prepare' {
            & (Join-Path $PSScriptRoot 'campaign3-prepare-action.ps1') -SemanticAction $a.semantic_action -WorkingCopy $a.working_copy `
        ...
        'subject' {
            ...
            $result = Invoke-ExternalJson $powershell @('-NoProfile','-ExecutionPolicy','Bypass','-File',
                (Join-Path $PSScriptRoot 'campaign3-subject-adapter.ps1'),'-RequestPath',$a.request_path,'-OutputPath',$a.output_path)
            if (-not $result.passed) { throw 'Campaign 3 subject invocation was invalid.' }
        }
        'material-action' {
            & (Join-Path $PSScriptRoot 'campaign3-recover-action.ps1') `
        ...
        default { throw "Unknown Campaign 3 worker operation: $($job.operation)" }
    }
    Write-Status ([ordered]@{
        schema = 'world-kernel-build001-campaign3-user-job-status-v1'
        ok = $true
